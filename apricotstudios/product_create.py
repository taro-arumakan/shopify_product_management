...
import string
import utils
from product_metafields_update_product_description import update_product_description_metafield
from product_metafields_update_size_table_html import update_size_table_html_metafield
from product_description_include_metafield_value import update_description_include_metafield_value
...
def product_info_list_from_sheet_color_and_size(gai:utils.Client, sheet_id, sheet_name):
    ...
    return gai.to_products_list(sheet_id, sheet_name, start_row, column_product_attrs,
                                option1_attrs, option2_attrs, row_filter_func=lambda row: row[string.ascii_lowercase.index('b')]=='5/16(1ST)')
...
def create_a_product(sgc:utils.Client, product_info, vendor, additional_tags=None):
    ...
    tags = ','.join([product_info['category'], product_info['release_date']] + additional_tags or [])
    ress = []
    ress.append(sgc.create_a_product(product_info=product_info, description_html='', vendor=vendor, tags=tags, location_names=['Apricot Studios Warehouse']))
    ress.append(process_images(sgc, product_info))
    ress.append(update_product_description_metafield(sgc, product_info['title'], product_info['description'], product_info['material'], product_info['made_in']))
    size_text = product_info['size_text']
    size_table_html = text_to_html_tables_and_paragraphs(size_text)
    ress.append(update_size_table_html_metafield(sgc, product_info['title'], size_table_html))
...
def create_products(sgc:utils.Client, product_info_list, vendor, additional_tags=None):
    ...
    for product_info in product_info_list:
        ress.append(create_a_product(sgc, product_info, vendor, additional_tags=additional_tags))
...
def process_images(sgc:utils.Client, product_info):
    image_pathss = [download_and_rename_images_from_dropbox(os.path.join(IMAGES_LOCAL_DIR, product_info['title']),
    ...
    for variant in product_info['options']:
        ...
        image_pathss += [download_and_rename_images_from_dropbox(os.path.join(IMAGES_LOCAL_DIR,
                                                                 product_info['title']),
    ...
    detail_image_paths = download_and_rename_images_from_dropbox(product_info['title'],
...
def main():
    ...
    product_info_list = product_info_list_from_sheet_color_and_size(client, client.sheet_id, 'Products Master')
    ress = create_products(client, product_info_list, vendor, additional_tags=['New Arrival', '25 Summer'])
    pprint.pprint(ress)
