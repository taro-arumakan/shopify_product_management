...
def main():
    ...
    for f in folders:
        ...
        if name != "R TRUNK FRAME ep.3 84L / 20’’" and " COVER " not in name:
            ...
            product = client.product_by_query(f"title:{name}*", additional_fields=['descriptionHtml'])
            if not product['descriptionHtml']:
                print(f'product {name} has no description, prrocessing')
                local_paths = client.drive_images_to_local(dir_id, IMAGES_LOCAL_DIR, filename_prefix=f'{prefix}_')
                if local_paths:
                    product_id = client.product_id_by_title(name.replace('_', '/'))
                    print(f'upload {len(local_paths)} images to {product_id}')
                    client.upload_and_assign_description_images_to_shopify(product_id, local_paths, DUMMY_PRODUCT,
