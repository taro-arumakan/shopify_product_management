...
THEME_BASE_DIR = "/Users/taro/sc/lememe/"
...
def find_thumbnail_image_name(page_info):
    cover_images_map = {
        "lookbook-fall": "top_lookbook_2025_fall.png",
        "lookbook-sample": "top_lookbook_2005_prefall.png",
    }
    return cover_images_map[page_info["handle"]]


def create_redirect(client: LememeClient, page_info, handle=""):
    client.create_url_redirect(
        f"/pages/{page_info['handle']}", f"/blogs/lookbook/{handle}"
    )


def process_page(client: LememeClient, page_info):
