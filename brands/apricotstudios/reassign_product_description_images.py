...
def main():
    ...
    for path in os.listdir(localdir):
        if "_product_main" in path:
            ...
            paths_by_product_title.setdefault(path.split("_")[0], []).append(
                f"{localdir}/{path}"
            )
    ...
    for product_title, paths in paths_by_product_title.items():
        ...
        other_paths = [
            f"{other_localdir}/{product_title}/{p}"
            for p in os.listdir(f"{other_localdir}/{product_title}")
            if "_product_detail_" in p
            and not p.endswith(".psd")
            and not p in ignore_names
        ]
        all_paths = sorted(
            paths + other_paths,
            key=lambda x: int(x.rsplit("/", 1)[-1].split("_")[-1].split(".")[0]),
        )
