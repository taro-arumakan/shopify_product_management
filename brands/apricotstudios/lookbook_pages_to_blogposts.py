...
THEME_NAME = "25AutumnProd"

THEME_BASE_DIR = "/Users/taro/sc/apricotstudios/"
COVER_PAGE_TEMPLATE_NAME = "lookbook-2"
...
def find_thumbnail_image_name(page_info):
    coverpage_path = os.path.join(
        THEME_BASE_DIR, "templates", f"page.{COVER_PAGE_TEMPLATE_NAME}.json"
    )
    with open(coverpage_path) as f:
        content = f.read()
        # Remove C-style comments
        content = re.sub(r"/\*.*?\*/", "", content, flags=re.DOTALL)
        d = json.loads(content)
    for section in d["sections"].values():
        if section["type"] == "multi-column":
            for block in section["blocks"].values():
                if (
                    block["settings"]["link_url"].rsplit("/", 1)[-1]
                    == page_info["handle"]
                ):
