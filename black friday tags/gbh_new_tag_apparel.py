...
df = pd.read_csv(r'/Users/taro/Downloads/GBH\ Japan\ EC_2024\ Black\ Friday\ -\ JP\ RRP\ \(판가\)\ APPAREL.csv'.replace('\\', ''), header=1)
...
for percentage in dict.fromkeys(df['BF 割引率'].to_list()):
    new_tag = f'2024 BF {percentage} OFF APPAREL'
...
assert not s[s > 1], 'multiple sets of tags for a product'

products = products[['Handle', 'Title', 'Tags']].drop_duplicates()
products.to_csv('/Users/taro/Downloads/gbh_2024bf_new_tags_apparel.csv', index=False)
