...
df = pd.read_csv(r'/Users/taro/Downloads/20241218_gbh_holiday_sales_list_HOME.csv', header=1)
df = df[['品番', 'ITEM', 'COLOR', 'SIZE', '年末年始\n割引率']]
df = df[(df['年末年始\n割引率'].notnull()) & (df['年末年始\n割引率'].str.endswith('%'))].reset_index(drop=True)
...
for percentage in dict.fromkeys(df['年末年始\n割引率'].to_list()):
    new_tag = f'2024 Holiday {percentage} OFF HOME'
...
products.to_csv('/Users/taro/Downloads/gbh_2024holiday_new_tags_home.csv', index=False)
