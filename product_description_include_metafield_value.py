...
def get_updated_description(sgc: utils.Client, product_id):
    ...
    metafield_product_description = sgc.product_metafield_value_by_product_id(
        product_id, "custom", "product_description"
    )
    metafield_product_description_converted = sgc.convert_rich_text_to_html(
        metafield_product_description
    )
    try:
        metafield_size_table_html = sgc.product_metafield_value_by_product_id(
            product_id, "custom", "size_table_html"
        )
    except TypeError:
        metafield_size_table_html = ""
    description_html = re.sub(
        f"<{hidden_description_element_old}>.*</{hidden_description_element_old}>",
        "",
        description_html,
        flags=re.DOTALL,
    )
