...
def get_product_description(desc, material, size_text, origin):
    ...
    res = {
        ...
        "children": [
            ...
            {
                "children": [{"type": "text", "value": size_text.strip('"').strip()}],
                "type": "paragraph",
            },
            {
...
def get_product_care(product_care_text):
    res = {
        ...
        "children": [
            {
                "children": [{"type": "text", "value": product_care_text.strip('"')}],
                "type": "paragraph",
            },
        ],
    }
...
def main():
    ...
    for row in rows[2:]:
        ...
        assert (
            product_description and product_care
        ), f"product_description or product_care is empty for {title}"
        product_id = client.product_id_by_title(title)
        res1 = client.update_product_description_metafield(
