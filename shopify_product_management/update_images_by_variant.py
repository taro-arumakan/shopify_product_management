...
from google_utils import drive_images_to_local, worksheet_rows, drive_link_to_id
import utils
...
def process(sgc:ShopifyGraphqlClient, sku, folder_id, google_credential_path, images_local_dir, upload_image_prefix):
    ...
    local_paths = drive_images_to_local(google_credential_path, folder_id, IMAGES_LOCAL_DIR, download_filename_prefix=f'{UPLOAD_IMAGE_PREFIX}_{sku}_')
    ...
    uploaded_variant_media = sgc.product_media_by_file_name(product_id, file_names[0])
    sgc.assign_image_to_skus(product_id, uploaded_variant_media['id'], [variant_id])


def main():
    cred = utils.credentials('rohseoul')
    rows = worksheet_rows(cred.google_credential_path, cred.sheet_id, '25SS 2차오픈(4월)(Summer 25)')
