...
ALL_BRANDS = ["gbhjapan", "kumej", "lememek", "rohseoul", "ssilkr", "blossomhcompany", "alvanas", "apricot-studios", "archive-epke"]


def get_product_rows(product, brand_name):
    """商品をCSV行リストに変換"""
    variants = (product.get("variants") or {}).get("nodes", []) if product.get("variants") else []
    media = (product.get("media") or {}).get("nodes", []) if product.get("media") else []
    main_img_url = (media[0].get("image") or {}).get("url", "") if media and media[0].get("image") else ""
    
    opt_names = ([o.get("name", "") for o in (variants[0].get("selectedOptions") or [])] if variants and variants[0].get("selectedOptions") else []) + [""] * 3
    rows = []
    for idx, v in enumerate(variants):
        ...
        rows.append({
            ...
            "Option1 Name": opt_names[0], "Option1 Value": opts[0], "Option1 Linked To": "",
            "Option2 Name": opt_names[1], "Option2 Value": opts[1], "Option2 Linked To": "",
            "Option3 Name": opt_names[2], "Option3 Value": opts[2], "Option3 Linked To": "",
            "Variant SKU": v.get("sku", ""), "Variant Grams": "", 
            "Variant Inventory Tracker": "shopify", "Variant Inventory Policy": "deny",
            "Variant Fulfillment Service": "manual", "Variant Price": v.get("price", ""),
            ...
            "Gift Card": "FALSE", "Variant Image": v_img, "Variant Weight Unit": "kg",
            "Status": "active" if product.get("status") == "ACTIVE" else "draft",
            **{k: "" for k in SHOPIFY_COLS if k not in ["Handle", "Title", "Body (HTML)", "Vendor", "Product Category", "Type", "Tags", "Published", "Option1 Name", "Option1 Value", "Option1 Linked To", "Option2 Name", "Option2 Value", "Option2 Linked To", "Option3 Name", "Option3 Value", "Option3 Linked To", "Variant SKU", "Variant Inventory Tracker", "Variant Inventory Policy", "Variant Fulfillment Service", "Variant Price", "Variant Compare At Price", "Variant Requires Shipping", "Variant Taxable", "Image Src", "Image Position", "Image Alt Text", "Gift Card", "Variant Image", "Variant Weight Unit", "Status"]}
        })
    return rows


def upload_to_drive(client, filepath, folder_id):
    """Google Driveへアップロード"""
    if not folder_id:
        return
    try:
        media = MediaIoBaseUpload(open(filepath, 'rb'), mimetype='text/csv', resumable=True)
        file = client.drive_service.files().create(
...
def process_brands(folder_id=None):
    ...
    for brand in ALL_BRANDS:
        try:
            ...
            filepath = os.path.join("exports", f"{brand}_products_{datetime.now():%Y%m%d}.csv")
            ...
            if folder_id:
                upload_to_drive(client, filepath, folder_id)
