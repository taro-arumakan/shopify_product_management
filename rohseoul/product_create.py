...
def product_info_lists_from_sheet(gai:utils.Client, sheet_id, sheet_name, handle_suffix):
    start_row = 2
...
def process_product_images(client:utils.Client, product_info, handle_suffix):
    ...
    product_id = client.product_id_by_handle('-'.join(product_info['title'].lower().split(' ') + [handle_suffix]))
    local_paths = []
    image_position = 0
...
def main():
    ...
    product_info_list = product_info_lists_from_sheet(client, client.google_sheet_id, '25SS 2차오픈(4월)(Summer 25)', handle_suffix)
