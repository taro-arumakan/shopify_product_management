...
def process(client:utils.Client, sku, folder_id):
    ...
    local_paths = client.drive_images_to_local(folder_id, IMAGES_LOCAL_DIR, download_filename_prefix=f'{UPLOAD_IMAGE_PREFIX}_{sku}_')
    ...
    client.assign_images_to_product([target['resourceUrl'] for target in staged_targets],
                                 alts=file_names,
                                 product_id=product_id)
    variant_id = client.variant_id_by_sku(sku)
    uploaded_variant_media = client.media_by_product_id_by_file_name(product_id, file_names[0])
