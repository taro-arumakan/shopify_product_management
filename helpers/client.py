    ...
    def process_product_images(
        ...
        for index, (drive_id, skus) in enumerate(zip(drive_links, skuss)):
            local_paths = self.drive_images_to_local(
                drive_id, local_dir, f"{local_prefix}{skus[0]}"
            )
            res = self.upload_and_assign_images_to_product(
                product_id, local_paths, remove_existings=index == 0
            )
            ress.append(res)
            ...
            ress.append(
                self.assign_image_to_skus(product_id, variant_media_id, variant_ids)
            )
        return ress
