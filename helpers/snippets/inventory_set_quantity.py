...
arhivepke_column_map = dict(sku_index=4, quantity_index=12, start_row=3)
kume_column_map = dict(sku_index=18, quantity_index=19, start_row=3)
...
def sku_quantity_map_from_sheet(shop_name, sheet_title):
    ...
    for row in rows[columns["start_row"] :]:
        assert (
            row[columns["sku_index"]] not in res
