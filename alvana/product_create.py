...
def populate_option(product_info, option1_key, option2_key):
    return [[{option1_key: option1[option1_key], option2_key: option2[option2_key]}, product_info['price'], option2['sku']]
...
def create_a_product(sgc:utils.Client, product_info, vendor):
    ...
    description_html = get_description(product_info['description'], product_info['material'], product_info['made_in'])
    ...
    res2 = [sgc.enable_and_activate_inventory(option2['sku'], []) for option1 in product_info['options'] for option2 in option1['options']]
    return (res, res2)
